...
<c:choose>
    <c:when test="${fn:length(products) gt 0}">
        ...
        <c:forEach var="product" items="${products}">
                <tr>
                    <td><input name="textfield" type="text" value="<c:out value="${product.id}"/>" disabled></td>
                    <td><input name="textfield" type="text" value="<c:out value="${product.name}"/>" disabled></td>
                    <td><a href="/product/delete?id=<c:out value='${product.id}' />">delete</a></td>
                </tr>
        </c:forEach>
        </tbody>
        </table>
